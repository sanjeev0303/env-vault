...
export default function RegisterPage() {
  ...
  const handleRegisterSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    registerMutation.mutate({ email: authEmail, password: authPassword, name: authName }, {
      onSuccess: () => {
        // After successful registration, direct to login
        router.push("/login");
      }
    });
  };
  ...
  return (
    ...
    >
      ...
      <div className="bg-vanilla-100 text-noir-900 rounded-xl shadow-2xl p-8 w-full max-w-md border border-vanilla-300 relative z-10">
        ...
        <h2 className="font-serif text-3xl font-bold text-center mb-6">Create Account</h2>
        <form onSubmit={handleRegisterSubmit}>
          <div className="mb-4">
            <label className="block text-sm font-semibold uppercase tracking-wider text-noir-600 mb-2">Name</label>
            <input 
              type="text" 
              value={authName}
